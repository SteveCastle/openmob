...
const resolvers = client => ({
    ...
    Mutation: {
        newCause: async (_, { Title, Slug, Summary }) => {
            const { ID: layoutID } = await client
                .CreateLayout()
                .sendMessage({ api: 'v1', item: { LayoutType: 'adb5a57a-b0ab-4022-8d14-bde3efbe5ad9' } });
            const {
                ID: homePageID,
            } = await client.CreateHomePage().sendMessage({
                api: 'v1',
                item: { Title: 'Home Page', Layout: layoutID },
            });
            const { ID: photoID } = await client
                .CreatePhoto()
                .sendMessage({
                    api: 'v1',
                    item: {
                        URI:
                            'https://punknaturalism.com/static/84e6008a85bd56160b3e6d31f23d428e/a5547/N33fe4iyc6.jpg',
                        Width: 400,
                        Height: 400,
                    },
                });
