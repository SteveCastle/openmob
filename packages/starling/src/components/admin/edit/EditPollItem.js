...
const GET_POLLITEM = gql`
  query getPollItemById($id: ID!) {
    getPollItem(ID: $id) {
      ID
      CreatedAt {
        seconds
...
function EditPollItem({ id }) {
  const {
    data: { getPollItem: item = {} },
    error,
    loading,
  } = useQuery(GET_POLLITEM, {
    variables: { id },
  })

  const updatePollItem = useMutation(UPDATE_POLLITEM)
