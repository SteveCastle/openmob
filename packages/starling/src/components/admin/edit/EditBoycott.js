...
const GET_BOYCOTT = gql`
  query getBoycottById($id: ID!) {
    getBoycott(ID: $id) {
      ID
      CreatedAt {
        seconds
...
function EditBoycott({ id }) {
  const {
    data: { getBoycott: item = {} },
    error,
    loading,
  } = useQuery(GET_BOYCOTT, {
    variables: { id },
  })

  const updateBoycott = useMutation(UPDATE_BOYCOTT)
...
EditBoycott.propTypes = {
  id: PropTypes.string,
}

export default EditBoycott
