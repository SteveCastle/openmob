...
func (s *shrikeServiceServer) GetPoll(ctx context.Context, req *v1.GetPollRequest) (*v1.GetPollResponse, error) {
	...
	if createdAt.Valid {
		poll.CreatedAt, err = ptypes.TimestampProto(createdAt.Time)
		if err != nil {
			return nil, status.Error(codes.Unknown, "createdAt field has invalid format-> "+err.Error())
		}
	}
	if updatedAt.Valid {
		poll.UpdatedAt, err = ptypes.TimestampProto(updatedAt.Time)
		if err != nil {
			return nil, status.Error(codes.Unknown, "createdAt field has invalid format-> "+err.Error())
		}
	}
...
func (s *shrikeServiceServer) ListPoll(ctx context.Context, req *v1.ListPollRequest) (*v1.ListPollResponse, error) {
	...
	querySQL := queries.BuildPollListQuery(req.Filters, req.Ordering, req.Limit)
