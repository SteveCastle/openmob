...
func (s *shrikeServiceServer) GetPollItem(ctx context.Context, req *v1.GetPollItemRequest) (*v1.GetPollItemResponse, error) {
	...
	if !rows.Next() {
		...
		return nil, status.Error(codes.NotFound, fmt.Sprintf("PollItem with ID='%d' is not found",
			req.ID))
	}
	...
	if rows.Next() {
		return nil, status.Error(codes.Unknown, fmt.Sprintf("found multiple PollItem rows with ID='%d'",
			req.ID))
	}
...
func (s *shrikeServiceServer) ListPollItem(ctx context.Context, req *v1.ListPollItemRequest) (*v1.ListPollItemResponse, error) {
	...
	// get PollItem list
	rows, err := c.QueryContext(ctx, "SELECT id, created_at, updated_at, title, poll FROM poll_item")
	if err != nil {
		return nil, status.Error(codes.Unknown, "failed to select from PollItem-> "+err.Error())
	}
	defer rows.Close()
