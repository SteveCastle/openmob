...
func (s *shrikeServiceServer) GetPetition(ctx context.Context, req *v1.GetPetitionRequest) (*v1.GetPetitionResponse, error) {
	...
	if createdAt.Valid {
		petition.CreatedAt, err = ptypes.TimestampProto(createdAt.Time)
		if err != nil {
			return nil, status.Error(codes.Unknown, "createdAt field has invalid format-> "+err.Error())
		}
	}
	if updatedAt.Valid {
		petition.UpdatedAt, err = ptypes.TimestampProto(updatedAt.Time)
		if err != nil {
			return nil, status.Error(codes.Unknown, "createdAt field has invalid format-> "+err.Error())
		}
	}
...
func (s *shrikeServiceServer) ListPetition(ctx context.Context, req *v1.ListPetitionRequest) (*v1.ListPetitionResponse, error) {
	...
	querySQL := queries.BuildPetitionListQuery(req.Filters, req.Ordering, req.Limit)
