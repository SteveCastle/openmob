...
func (s *shrikeServiceServer) GetVoter(ctx context.Context, req *v1.GetVoterRequest) (*v1.GetVoterResponse, error) {
	...
	if createdAt.Valid {
		voter.CreatedAt, err = ptypes.TimestampProto(createdAt.Time)
		if err != nil {
			return nil, status.Error(codes.Unknown, "createdAt field has invalid format-> "+err.Error())
		}
	}
	if updatedAt.Valid {
		voter.UpdatedAt, err = ptypes.TimestampProto(updatedAt.Time)
		if err != nil {
			return nil, status.Error(codes.Unknown, "createdAt field has invalid format-> "+err.Error())
		}
	}
...
func (s *shrikeServiceServer) ListVoter(ctx context.Context, req *v1.ListVoterRequest) (*v1.ListVoterResponse, error) {
	...
	querySQL := queries.BuildVoterListQuery(req.Filters, req.Ordering, req.Limit)
