...
func (s *shrikeServiceServer) CreateLayoutRow(ctx context.Context, req *v1.CreateLayoutRowRequest) (*v1.CreateLayoutRowResponse, error) {
	...
	err = c.QueryRowContext(ctx, "INSERT INTO layout_row (layout, container, weight) VALUES($1, $2, $3)  RETURNING id;",
		req.Item.Layout, req.Item.Container, req.Item.Weight).Scan(&id)
	if err != nil {
...
func (s *shrikeServiceServer) GetLayoutRow(ctx context.Context, req *v1.GetLayoutRowRequest) (*v1.GetLayoutRowResponse, error) {
	...
	if err := rows.Scan(&layoutrow.ID, &createdAt, &updatedAt, &layoutrow.Layout, &layoutrow.Container, &layoutrow.Weight); err != nil {
		return nil, status.Error(codes.Unknown, "failed to retrieve field values from LayoutRow row-> "+err.Error())
	}

	// Convert pq.NullTime from database into proto timestamp.
	if createdAt.Valid {
		layoutrow.CreatedAt, err = ptypes.TimestampProto(createdAt.Time)
		if err != nil {
			return nil, status.Error(codes.Unknown, "createdAt field has invalid format-> "+err.Error())
		}
	}
	if updatedAt.Valid {
		layoutrow.UpdatedAt, err = ptypes.TimestampProto(updatedAt.Time)
		if err != nil {
			return nil, status.Error(codes.Unknown, "createdAt field has invalid format-> "+err.Error())
		}
	}
...
func (s *shrikeServiceServer) ListLayoutRow(ctx context.Context, req *v1.ListLayoutRowRequest) (*v1.ListLayoutRowResponse, error) {
	...
	querySQL := queries.BuildLayoutRowListQuery(req.Filters, req.Ordering, req.Limit)
...
func (s *shrikeServiceServer) UpdateLayoutRow(ctx context.Context, req *v1.UpdateLayoutRowRequest) (*v1.UpdateLayoutRowResponse, error) {
	...
	res, err := c.ExecContext(ctx, "UPDATE layout_row SET layout=$2, container=$3, weight=$4 WHERE id=$1",
		req.Item.ID, req.Item.Layout, req.Item.Container, req.Item.Weight)
	if err != nil {
